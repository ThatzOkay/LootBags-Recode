...
public class InventoryManager {
    ...
    public static Inventory lootBagsShop = Bukkit.createInventory(null, 18, LootBags.getLootBags().format("&5&lLootBags Shop"));

    public static void setItems(LootBags lb) {
        lootBagAmount = 1;
        lootBagsShop.clear();
        OnInvClick.priceLore(lb);
        remove10Meta.setDisplayName(lb.format("&4Remove 10"));
        remove10.setDurability((short) 14);
        remove10.setItemMeta(remove10Meta);
        lootBagsShop.setItem(1, remove10);
        remove5Meta.setDisplayName(lb.format("&4Remove 5"));
        ...
        remove1Lore.add(lb.format("&4Remove 1 Lootbags"));
        remove1Meta.setDisplayName(lb.format("&4Remove 1"));
